...
const DB_FILE =
  process.env.DB_FILE || path.join(__dirname, "logicontrol.db");
  ...
  async (req, res) => {
    try {
      const [
        ...
        get(
          "SELECT COUNT(*) n FROM produtos WHERE ativo=1"
        ),
        get(
          "SELECT COUNT(*) n FROM clientes WHERE ativo=1"
        ),
        get(
          ...
          async () => {
            ...
            for (const item of itens) {
              ...
              if (
                ...
                throw new Error(
                  `Estoque insuficiente para ${product.nome}. Disponível: ${product.quantidade}.`
                );
              }
